...
def SolveEq(Ndop,doptype,temp,defect,**kwarg):
    ...
    T = temp
    defectlist = defect
    ni, Na, Nd, ve, vp, miu_tot, miu_h, miu_e = getsomeparam(Ndop=Ndop,doptype=doptype,temp=T)
    ...
    while diff > tol:
        ...
        Natot = Na + Natot1
        if Ndtot >= Natot:
            n00 = 0.5*((Ndtot - Natot)+np.sqrt((Ndtot - Natot)**2+4*ni**2))
            p00 = ni**2 / n00
        elif Ndtot < Natot:
            p00 = 0.5*((Natot - Ndtot)+np.sqrt((Natot - Ndtot)**2+4*ni**2))
            n00 = ni**2 / p00
        fnlist = [(x['se'] * ve*n00 + x['sp'] * vp*ni * np.exp(-x['Et'] / kb / T)) / (x['se']*ve * (n00 + ni *
                                                                                          np.exp(x['Et'] / kb / T)) + x['sp']*vp * (p00 + ni * np.exp(-x['Et'] / kb / T))) for x in defectlist]
...
def SolveSS(Ndop,doptype,temp,defect,dplist,**kwarg):
    ...
    dplist=dplist
    ni, Na, Nd, ve, vp, miu_tot, miu_h, miu_e = getsomeparam(Ndop=Ndop,doptype=doptype,temp=T)
    n00, p00, nt00, f0list = SolveEq(Ndop=Ndop,doptype=doptype,temp=T,defect=defectlist)
    ...
    for i in range(dplist.shape[0]):
        ...
        ntcharge = 0
        while diff > tol:
            fnnlist = [(x['se'] * ve*n + x['sp'] * vp*ni * np.exp(-x['Et'] / kb / T)) / (x['se']*ve * (n + ni *
                                                                                              np.exp(x['Et'] / kb / T)) + x['sp']*vp * (p + ni * np.exp(-x['Et'] / kb / T))) for x in defectlist]
        ...
        USRHlist = [(n * p - ni**2) / ((n + ni * np.exp(d['Et'] / kb / T)) / d['Nt'] / d['sp'] /
                                       vp + (p + ni * np.exp(-d['Et'] / kb / T)) / d['Nt'] / d['se'] / ve) for d in defectlist]
        Utot = sum(USRHlist)
        if doptype=='p':
            tau_eff = (n - n00) / Utot
        elif doptype=='n':
    ...
    for i in range(len(defectlist)):
        ...
        plt.plot([dplist[0],dplist[-1]],[f0list[i]*defectlist[i]['Nt'],f0list[i]*defectlist[i]['Nt']],'--',color=color)
        plt.plot(dplist,np.array(allntlist)[:,i],color=color,label='nt'+str(i+1))
    plt.plot(dplist,dnapplist,label='dnapp')
    plt.plot(dplist,Nd-Na-nlist+plist+ntchargelist,'.',label='charge neutrality')
    plt.grid(True)
    plt.legend(loc=0)
    ...
    for i in range(len(defectlist)):
        plt.plot(dplist,np.array(allntlist)[:,i]-f0list[i]*defectlist[i]['Nt'],label='delta nt'+str(i+1))
...
def SolveTrans(Ndop,doptype,temp,defect,d0,t,**kwarg):
    Ndop = Ndop
    doptype = doptype
    T = temp
    defectlist = defect
    ...
    def trapode(y, t):
        ...
        Reimp = [x['se'] * ve * n * (x['Nt'] - y[defectlist.index(x)+2]) for x in defectlist]
        Geimp = [x['se'] * ve * ni * np.exp(x['Et'] / kb / T) * y[defectlist.index(x)+2] for x in defectlist]
        Rhimp = [x['sp'] * vp * p * y[defectlist.index(x)+2] for x in defectlist]
        Ghimp = [x['sp'] * vp * ni * np.exp(-x['Et'] / kb / T) * (x['Nt'] - y[defectlist.index(x)+2]) for x in defectlist]

        dydt = [-sum(Reimp)+sum(Geimp),-sum(Rhimp)+sum(Ghimp)]
    ...
    for i in range(len(defectlist)):
        color = next(ax._get_lines.prop_cycler)['color']
        plt.plot([t[1],t[-1]],[f0list[i]*defectlist[i]['Nt'],f0list[i]*defectlist[i]['Nt']],'--',color = color)
    ...
    ax = plt.gca()
    for i in range(len(defectlist)):
        plt.plot(t,nt[:,i]-f0list[i]*defectlist[i]['Nt'],label='delta nt'+str(i+1))
...
SolveTrans(Ndop=Ndop,doptype=doptype,temp=300,defect=defectlist,d0=d0,t=t)
